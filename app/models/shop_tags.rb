module ShopTags
  ...
  tag 'product:find' do |tag|
    ...
    product=ShopProduct.find(:first, :conditions => where)
    if product then
  ...
  tag 'products:each' do |tag|
    ...
    if tag.locals.category || tag.locals.subcategory then
      ...
      prods=ShopProduct.find(:all, :conditions => where, :order => order)
    end
  ...
  %w( description filename ).each do |field|
    tag "product:images:#{field}" do |tag|
  ...
  tag 'category:find' do |tag|
    ...
    category=ShopCategory.find(:first, :conditions => where)
    if category then
  ...
  tag 'categories:each' do |tag|
    ...
    if attr[:parent] then
      if attr[:parent] =~ /^\d+$/ then
        ...
        parent=ShopCategory.find(:first, :conditions => { :title => attr[:parent] })
        parent_id=parent.id
      end
    ...
    result = []
    if attr[:parent] then
      cats=ShopCategory.find(:all, :conditions => where, :order => order)
    else
      cats=ShopCategory.find_all_top_level(:conditions => where, :order => order)
    end
