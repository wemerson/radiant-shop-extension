...
class ShopCartExtension < Radiant::Extension
  ...
  define_routes do |map|
    map.namespace 'shop' do |shop|
      ...
      shop.cart_items 'cart/items', :controller => 'line_items', :action => 'index', :order_id => 'current', :conditions => { :method => :get } 
      shop.cart_item_add 'cart/items', :controller => 'line_items', :action => 'create', :order_id => 'current', :conditions => { :method => :post } 
      shop.cart_item_remove 'cart/items/:id/remove', :controller => 'line_items', :action => 'destroy', :order_id => 'current'
